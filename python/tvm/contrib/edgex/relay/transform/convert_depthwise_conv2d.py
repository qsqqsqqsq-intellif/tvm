...
"""Relay transformations to handle group conv2d."""
import numpy as np
...
class DepthwiseConv2DConvertor(ExprMutator):
    ...
    def visit_call(self, call):
        ...
        if call.op == tvm.relay.op.get("nn.conv2d"):
            ...
            if data_layout == "NCHW" and is_depthwise_conv2d(
                ...
                groups_new = 1
                if groups % 16 == 0:
                    groups_new = groups // 16
                channel_per_group = kernel_shape[0] // groups_new
                kernel_shape[0] = groups_new * channel_per_group
                kernel_shape[1] = channel_per_group
                weight_new = np.zeros(kernel_shape, dtype=weight_old.dtype)
                for i in range(groups_new):
                    for j in range(channel_per_group):
                        oc_idx = i * channel_per_group + j
                        weight_new[oc_idx, j] = weight_old[oc_idx, 0, :, :]
                ...
                if self._is_irmod:
                    ...
                    new_attr_dict.update({"groups": groups_new})
