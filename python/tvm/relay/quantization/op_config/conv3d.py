...
class Conv3D:
    ...
    def __init__(self, node, vertex_config, config):
        ...
        LOGGER.debug("[analyze] conv3d start")
        self.quantized = True
        ...
        """input0-config"""
        # conv3d input0_axis
        conv3d_groups = node.attrs.groups
        ...
        LOGGER.debug("[analyze] conv3d finish")
    ...
    def quantize_params(self, node, vertex_config):
        """quantize_params"""
        conv_scale = []
        LOGGER.debug("[calibrate]conv3d nobias start and quantized is %d", self.quantized)
    ...
    def realize(self, old_node, new_node, vertex_config, n2o):
        """realize"""
        LOGGER.debug("[realize]conv3d nobias start...")
        ...
        if self.quantized:
            new_node = relay.nn.conv3d(realized_args[0], realized_args[1], **attrs)
        ...
        new_node = relay.nn.conv3d(realized_args_n[0], realized_args_n[1], **attrs)
        return new_node
