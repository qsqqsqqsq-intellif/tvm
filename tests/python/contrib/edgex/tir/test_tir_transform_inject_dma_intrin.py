                ...
                for c_o_i, h_i, w_i in T.grid(4, 17, 17):
                    if 1 <= 0*16 + h_i and 1 <= 0*16 + w_i:
                        for c_i in T.serial(0, 8):
                            dm[c_o_i*2312 + h_i*136 + w_i*8 + c_i] = \
                                X[c_o_o*1605632 + c_o_i*401408 + c_i*50176 + h_o*3584 + h_i*224 + w_o*16 + w_i - 225]
            with T.attr("", "pragma_nnp_dma_scope", "eodma"):
                for c_i, h_i, w_i in T.grid(32, 8, 8):
                    Y[c_o_o*401408 + c_i*12544 + h_o*896 + h_i*112 + w_o*8 + w_i] = \
                        dm[T.floordiv(c_i, 8)*512 + h_i*64 + w_i*8 + T.floormod(c_i, 8)]
    ...
    for c_o_o, h_o, w_o in T.grid(2, 14, 14):
        if ((h_o == 0) and (w_o == 0)):
            T.evaluate(T.nnp_eidma_load("int32",
                T.tvm_access_ptr(T.type_annotation(dtype="int32"), dm.data, 144, 9104, "w", dtype="handle"),
                ...
                "ei_state_num=1", "ei_crop=1", "ei_ub_data_size=32768", "ei_wo_data_size=32768", 
                "ei_dtype=4", "ei_mode=2",
            ...
            T.evaluate(T.nnp_eodma_store("int32",
                ...
                T.tvm_access_ptr(T.type_annotation(dtype="int32"), dm.data, 0, 2048, "r", dtype="handle"),
                "eo_start_addr_in_en=1", "eo_start_addr_out_en=1", "eo_first_state_en=1",
                "eo_state_num=3", "eo_crop=1", "eo_ub_data_size=8192", "eo_wo_data_size=8192",
                "eo_dtype=4", "eo_mode=1",
...
def eidma_1D_broadcast_pattern_rewritten(data: T.handle) -> None:
    ...
    T.evaluate(T.nnp_eidma_load("int32",
        T.tvm_access_ptr(T.type_annotation(dtype="int32"), dm.data, 0, 16, "w", dtype="handle"),
        ...
        "ei_crop=1", "ei_ub_data_size=64", "ei_wo_data_size=64",
...
def eidma_multi_branch_under_scope(data: T.handle) -> None:
    ...
    with T.attr("", "pragma_nnp_dma_scope", "eidma"):
        ...
        for i in range(4):
            ...
            # j in [4, 16)
            for j in range(12):
                dm[i * 16 + j + 4] = X[i * 16 + j + 4]
        # i in [4, 16)
        for i in T.serial(0, 12):
            # j == 0
            dm[i * 16 + 64] = X[i * 16 + 64]
            # j in [1, 15)
            for j in range(14):
                dm[i * 16 + j + 65] = X[i * 16 + j + 65]
            # j == 15
            dm[i * 16 + 79] = X[i * 16 + 79]

@T.prim_func
def eidma_multi_branch_under_scope_rewritten(X: T.Buffer[(256,), "int32"]) -> None:
    dm = T.allocate([256], "int32", "dm")
    T.evaluate(T.nnp_eidma_load("int32",
        T.tvm_access_ptr(T.type_annotation(dtype="int32"), dm.data, 0, 52, "w", dtype="handle"),
    ...
    T.evaluate(T.nnp_eidma_load("int32",
        T.tvm_access_ptr(T.type_annotation(dtype="int32"), dm.data, 4, 60, "w", dtype="handle"),
        T.tvm_access_ptr(T.type_annotation(dtype="int32"), X.data, 4, 60, "r", dtype="handle"),
        "ei_start_addr_in_en=1", "ei_start_addr_out_en=1", "ei_first_state_en=1", "ei_state_num=1",
        "ei_crop=1", "ei_ub_data_size=192", "ei_wo_data_size=192",
        "ei_dtype=4", "ei_mode=2",
        ...
        "ei_j0_strideout=256", "ei_j1_strideout=256", "ei_j2_strideout=64", dtype=""))
    T.evaluate(T.nnp_eidma_load("int32",
        T.tvm_access_ptr(T.type_annotation(dtype="int32"), dm.data, 64, 177, "w", dtype="handle"),
        T.tvm_access_ptr(T.type_annotation(dtype="int32"), X.data, 64, 177, "r", dtype="handle"),
        "ei_start_addr_in_en=1", "ei_start_addr_out_en=1", "ei_first_state_en=1", "ei_state_num=1",
        "ei_crop=1", "ei_ub_data_size=48", "ei_wo_data_size=48", "ei_dtype=4", "ei_mode=2",
    ...
    T.evaluate(T.nnp_eidma_load("int32",
        T.tvm_access_ptr(T.type_annotation(dtype="int32"), dm.data, 65, 190, "w", dtype="handle"),
        T.tvm_access_ptr(T.type_annotation(dtype="int32"), X.data, 65, 190, "r", dtype="handle"),
        "ei_start_addr_in_en=1", "ei_start_addr_out_en=1", "ei_first_state_en=1", "ei_state_num=1",
        "ei_crop=1", "ei_ub_data_size=672", "ei_wo_data_size=672", "ei_dtype=4", "ei_mode=2",
        "ei_j0_loop_num=1", "ei_j1_loop_num=1", "ei_j2_loop_num=12", "ei_j3_loop_num=14",
        "ei_j0_loop_sel=3", "ei_j1_loop_sel=2", "ei_j2_loop_sel=1", "ei_j3_loop_sel=0",
        "ei_j0_stridein=768", "ei_j1_stridein=768", "ei_j2_stridein=64",
        "ei_j0_strideout=768", "ei_j1_strideout=768", "ei_j2_strideout=64", dtype=""))
    T.evaluate(T.nnp_eidma_load("int32",
        T.tvm_access_ptr(T.type_annotation(dtype="int32"), dm.data, 79, 177, "w", dtype="handle"),
